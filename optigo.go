...
func (o *OptionParser) initResultKey(opt *option) {
	if _, ok := o.Results[opt.name]; ok {
		return
	}
	var dflt interface{}
	if opt.unary {
		...
		if opt.action == atAPPEND {
			...
			case dtINTEGER:
				dflt = make(map[string]int64)
			case dtFLOAT:
				dflt = make(map[string]float64)
			}
		} else {
			switch opt.dataType {
			case dtSTRING:
